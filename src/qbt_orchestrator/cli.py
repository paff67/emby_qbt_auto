...
def _build_runtime(ns, db: Path, force_dry_run: bool | None = None) -> tuple[DaemonRuntime, bool]:
    ...
    full_cleanup_enabled = bool(_truthy(os.environ.get("QBT_ORCH_FULL_CLEANUP")) or False)
    full_cleanup_dry_env = _truthy(os.environ.get("QBT_ORCH_FULL_CLEANUP_DRY_RUN"))
    full_cleanup_dry_run = True if dry_run else (full_cleanup_dry_env if full_cleanup_dry_env is not None else True)
    runtime = DaemonRuntime(
        ...
        full_cleanup_enabled=full_cleanup_enabled,
